...
function ProfileFollowLists (props) {
    ...
    useEffect(props => {
        ...
        const fetchFollowLists = async function (props) {
            try {
      ...
      <div className="list-group">
        ...
        {!Boolean(state.followList.length) && (
          <div className="alert alert-danger text-center shadow-sm">
            ...
            <p>This is a surprise, {username} has no-one on their {props.type} list.</p>
            <p>Maybe you could be {username}'s first {props.type}</p>
          </div>
        )}
      </div>
    );
